...
class Myfigure(FigureCanvas):
    def __init__(self):
        ...
        FigureCanvas.__init__(self, self.fig) #初始化**widget中的plt部分

    def Draw(self):
        global C_x, C_y, D_d
        plt.imshow(self.draw_mandlbrot(C_x, C_y, D_d, 400, 400))
        self.draw()
        self.fig.canvas.mpl_connect('button_press_event', self.on_press) #如果不使用canvas.mpl_connect的话将不能**event中的xdata和ydata以及inaxes，这里为相对axes的坐标
        # pos = plt.ginput(3)
        # print(pos)
